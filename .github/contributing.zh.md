...
- Quora 链接需要身份验证，对匿名访问返回 `403`：不要使用，请寻找替代来源。

- 当 GitHub 仓库已转移到新所有者时，请将 URL 更新为新的规范位置，而不是依赖重定向（旧的 `OWNER/REPO` 命名空间可被任何人重新认领）。

- 当供应商博客消失时（被收购、品牌重塑、子域停用），请用 `https://web.archive.org/web/<YYYYMMDDhhmmss>/` 包装原始 URL，而不是使用模糊的年份前缀：具体的时间戳比让 Wayback 选择最近快照更耐久。
...
- 当项目的主要维护者是销售此软件付费版本的商业供应商时，请在描述前加上 💸 表情符号：托管云、企业层或位于开源核心之上的专有模块。该标记是信息性的，而非质量判断：它告诉读者某些高级功能（合规性、审计日志保留、多租户附加组件、集成）可能位于开源发行版之外。格式：`- [Project](url) - 💸 描述。`。当供应商仅销售支持、SLA 或托管而不进行功能门控时（例如 Red Hat 对 Keycloak），请勿应用此标记，对于没有商业支持者的社区驱动项目也不适用。

- 当项目是完全开源的，没有商业供应商通过功能门控提取价值时，请在描述前加上 🆓 表情符号：基金会治理（CNCF、Apache、Eclipse）、企业开源在没有附带付费产品的情况下发布（Lyft 的 Cartography、Yahoo 的 Athenz、Salesforce 的 Policy Sentry、Microsoft 的 Presidio）、社区驱动，或仅销售支持合同（Red Hat 对 Keycloak）。格式：`- [Project](url) - 🆓 描述。`。💸 和 🆓 标记是互斥的：一个项目只能有其中一个，不能同时拥有两者。文章和论文条目不会被标记。
...
对于 IAM 项目，以下功能被视为**基本功能**，而非可选附加组件：

- MFA（TOTP、WebAuthn、推送）
- SSO 协议（SAML、OIDC、LDAP/AD 联合）
- 多租户或组织
- RBAC、细粒度权限
- SCIM 或目录同步
- 管理 UI 和审计日志
...
- 供应商的定价页面将看似核心 IAM 功能列为付费层。
- 项目 README 明确提到云端保留的"保护屏障"或"高级功能"。
- 自托管开源版本需要组装多个供应商仅作为付费产品预集成的独立原语。

通过许可证测试但属于 open-core-light（部分高级合规或目录同步集成被门控，但核心 IdP 在开源中可用）的现有条目，在覆盖独特领域时仍可列出。

### 为什么我的项目因不活跃而被删除？

除了 [已存档的仓库](#为什么要移除不活跃的-github-项目) 之外，我们还会删除作者已正式退役的项目：README 中将用户指向其他项目的通知，或者"新项目不应再依赖此项目"的公开声明，被视为比单独的 GitHub 存档标志更强的信号。
